...
from app.schemas.recanime.predict import AnimeAttributes, PredictResults, AnimeInfo
...
def create_buttons():
    ...
    button_cols[0].button(
        label='Reset', key=component_keys.reset_button_key,
        on_click=handle_reset_button_click,
    )
    button_cols[1].button(
        label='Predict', key=component_keys.predict_button_key,
...
def handle_predict_button_click():
    ...
    results: List[PredictResults] = recanime_api.predict_top_k_animes(
        anime_attributes=anime_attributes,
        top_k=10,
        timeout=3
    )
